...
# ------------------------------
# Progress publisher for WebSocket
# ------------------------------
def publish(job_id, phase, percent, message):
    r.publish(
        f"progress_{job_id}",
        json.dumps({"phase": phase, "percent": percent, "message": message}),
    )
...
def process_pdf(pdf_path, job_id):
    """
    Safe, single-threaded indexing task for a PDF.
    """
    ...
    for i in range(1, pages + 1):
        publish(
            job_id, "extraction", int(i / pages * 100), f"Extracted page {i}/{pages}"
        )
    print("Text extracted to:", txt_path)
    ...
    for i in range(1, total_chunks + 1):
        publish(
            ...
            f"Indexing chunk {i}/{total_chunks}",
        )

    index_chunks(chunks, source_name=os.path.basename(pdf_path), job_id=job_id)
    publish(job_id, "indexing", 100, f"Indexing complete! Total chunks: {total_chunks}")
